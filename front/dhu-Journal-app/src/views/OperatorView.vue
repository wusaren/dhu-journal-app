        ...
        <div v-if="currentBatch" class="batch-detail">
          ...
          <el-descriptions :column="3" border size="small">
            ...
            <el-descriptions-item label="输出目录">
              <span style="word-break: break-all;">{{ currentBatch.output_path }}</span>
              <el-button
                size="small"
                type="info"
                style="margin-left: 8px;"
                @click="handleOpenBatchFolder"
              >
...
const handleOpenBatchFolder = async () => {
  ...
  try {
    const response = await operatorService.openBatchFolder(currentBatch.value.id)
    if (!response.success) {
